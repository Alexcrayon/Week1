...
public class Part2AndPart3 {
	...
	public static void printArray(String[][] array) {
		for (int i = 0; i < array.length; i++) {
			for (int j = 0; j <= array[0].length - 1; j++) {
				if (j == 0) {// align the first columns(names)
	...
	// print one of the rows from the array
	public static void printRow(String[][] array, int row) {
		for (int i = 0; i <= array[0].length - 1; i++) {
			if (i == 0) {// align the first columns(names)
				System.out.printf("%-20s", array[row][i]);
			} else
	...
	public static void main(String[] args) {
		...
		String customers[] = new String[] { "Amy Garrison", "Braydon Beil", "Bluebell Nguyen", "Konrad Sears",
		...
		double payments[][] = new double[][] { { 1032, 47, 43, 38, 61, 69, 99, 47, 36, 30, 99, 59, 17 },
		...
		// create a 50*15 array to store customers, payments and standings
		String[][] report = new String[50][15];
		// copy the data from customers array to the new array
		for (int i = 0; i < 50; i++) {
			report[i][0] = customers[i];
		}
		// copy the data from payments to the new array
		for (int i = 0; i < 50; i++) {
			int standing = 0;
			for (int j = 1; j < 14; j++) {
				report[i][j] = String.valueOf((int) payments[i][j - 1]);
				// evaluate the standing of the account depending
				// how many zero payments it has and store it to the new array
				if (payments[i][j - 1] == 0) {
					standing++;
				}
				if (standing == 0) {
		...
		do {
			...
			switch (choice) {
			// menu options
			case 1:// find the account by the input account number
				int accountNum;
				System.out.println("Enter the account number:");
				accountNum = input.nextInt();
				System.out.print("Result:\n");
				for (int i = 0; i < report.length; i++) {
					if (accountNum == Integer.parseInt(report[i][1])) {
						printRow(report, i);
					}
				}
				break;
			// find those accounts aren't good standing(accounts with zero payments)
			case 2:
				System.out.print("Result:\n");
				for (int i = 0; i < report.length; i++) {
					// check if string are equal
					if (!report[i][14].equals("Good")) {
						printRow(report, i);
					}
				}
				break;
			// find the accounts which have been closed
			case 3:
				System.out.print("Result:\n");
				for (int i = 0; i < report.length; i++) {
					if (report[i][14].equals("Closed")) {
						printRow(report, i);
					}
				}
				break;
